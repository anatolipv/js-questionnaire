const questions = [
  ...
  },
  {
    ...
    answer:
      'An array is a secquence of elements in order. Each element has its way to be accessed-index that starts from zero.',
    id: 2,
  },
  // Functions / Exceptions
  {
    module: 'Core',
    theme: 'functions-exceptions',
  ...
  },
  {
    ...
    answer:
      'With function declaration, function can be used even before its declaration which is not the case with arrow or expressions; arrow func does not have argiments obj and cant use <this>; different sintax',
    id: 4,
  },
  {
    module: 'Core',
    ...
    answer:
      'An IIFE (Immediately Invoked Function Expression) is a JavaScript function that runs as soon as it is defined. We use it for several reasons: 1.Avoid polluting the global namespace, 2.Execute an async function, 3.The module pattern (create private and public variables and methods) ',
    id: 5,
  },
  // NPM / ES Modules
  {
    ...
    answer:
      'NPM - Node Package Manager is the default package manager for JavaScript runtime enviroment Node.js. All the packages are open source(we can check the code and see how it works), the most used of each kind are well tested and prowen to work, most of the packages are updated regularly. In order to use NPM we need to have Node.js installed.',
    id: 6,
  },
  {
    module: 'Core',
    ...
    answer:
      'In order to be able to update,add or remove packages NPM stores all the information about the project and used packages in the package.json file.',
    id: 7,
  },
  {
    module: 'Core',
  ...
  },
  {
    ...
    answer:
      'ES Modules came with ECMAScript6(ES2015). This is modern way to do modules in JavaScript. This approach uses the export and import keywords, instead of the older CommonJS syntax of module. exports and require.',
    id: 9,
  },
  // ESNext / ESLint
  {
    ...
    answer:
      'Strict mode makes it easier to write secure JavaScript. Strict mode changes previously accepted bad syntax into real errors. As an example, in normal JavaScript, mistyping a variable type creates a new global variable.',
    id: 10,
  },
  {
    module: 'Core',
    ...
    answer:
      'Multiline strings with value placeholder without the use of endline operator `${name} is ${age} years old.`. No need to concatenate strings.',
    id: 11,
  },
  {
    module: 'Core',
    ...
    answer:
      'Destructuring assignments allow us to extract values from objects or arrays.',
    id: 12,
  },
  //  Objects and destructuring
  {
    module: 'Core',
    theme: 'obj-destructuring',
    question: 'What are objects? Provide an example.',
    answer:
      'An object is an instance which contains a set of key value pairs. Compare it with a cup, for example. A cup is an object, with properties. A cup has a color, a design, weight, a material it is made of, etc. The same way, JavaScript objects can have properties, which define their characteristics.',
    id: 13,
  },
  {
    module: 'Core',
  ...
  // Git
  {
    ...
    answer:
      'Git is the most widely used morern version control system in the world. Some of the benefits: 1.Every developer has a working copy of the code and the repository, 2.Performance, security and flexibility in mind.',
    id: 17,
  },
  {
    module: 'Core',
  ...
  },
  {
    ...
    answer:
      'GitLab is build ot top of Git.GitLab is a service to host your central repository online.',
    id: 19,
  },
  {
    module: 'Core',
    ...
    answer:
      'For authentication we need to set our global git username, email and verify it with our password in order to see who made the commit (or they will be anonymous).',
    id: 20,
  },
  {
    module: 'Core',
  ...
  },
  {
    ...
    answer:
      'In the context of scope, const and let are the same and have block scope and function scope where var has only function scope.',
    id: 23,
  },
  {
    module: 'Core',
    ...
    answer:
      'Both `==` and `===` are comparison operators used in logical statements to redermine equality or difference between variables or values.`===` compares values and types and `==` comperes only values(using type coercion if needed). Type coercion is the automatic or implicit conversion of values from one data type to another.Example: 5 == `5` (true), 5 === `5` (false).',
    id: 24,
  },
  {
    module: 'Core',
    ...
    answer:
      'Hoisting it the process of collecing variables by the compiler and later there will be a value attached to them.',
    id: 25,
  },
  {
    module: 'Core',
    ...
    answer:
      'Only the declaration is hoisted.Var variables and function declaration are accessible even before they are declared but const and let are in Temporal Dead Zone because they define block scope and are useble only after they are declered or initialized',
    id: 26,
  },
  {
    module: 'Core',
    ...
    answer:
      "Lexical scope is the definition area of an expression. In other words, an item's lexical scope is the place in which the item got created (The place an item got invoked/called is not necessarily the item`s lexical scope. Instead, an item`s definition space is its lexical scope).",
    id: 27,
  },
  {
    module: 'Core',
  ...
  {
    module: 'Core',
    theme: 'modulePattern-FPIntro',
    question: 'What is functional programming?',
    answer:
      'Functional programing is a programing paradigm or pattern which treats computatuins as the evaluation of mathemathical functions and avoids changing-state and mutable data.',
    id: 29,
  },
  {
    module: 'Core',
  ...
  },
  {
    ...
    answer:
      'It is the same as module pattern in which we reveal public pointers to methods inside the Module`s scope. In the revealing module pattern it is easier to see what is exposed without all the code to be in the returned object.',
    id: 32,
  },
  // Pure array functions
  {
    module: 'Core',
    theme: 'pureArrayFunctions',
    question:
      'What are the benefits of using array methods (forEach, map, etc.) over loops?',
    answer:
      'Code looks cleaner, no need to write common logic`s for simple array operations, less places to make misstakes (start/end point,increment/decrement etc.',
    id: 33,
  },
  {
    module: 'Core',
  ...
  {
    module: 'Core',
    theme: 'objectsAdvanced',
    question:
      'What is the main difference between reference copy and shallow copy?',
    answer:
      'Reference copy gives us another way to access the same object, with shallow copy a new object is created that has an exact copy of the velues in the original object. If any of the fields of the object are references to other objects, just the reference addresses are copied.',
    id: 36,
  },
  {
    module: 'Core',
    ...
    answer:
      'Deep copy copies all fields, and makes copies of dynamically allocated memory poited to by the fields. A deep copy occurs when an object is copied along with the objects to which it refers.',
    id: 37,
  },
  {
    module: 'Core',
    ...
    answer:
      'For complex data we use objects with key/value pairs and if needed a lot of nesting. Keys of object must be strings, but property values can be anything (another object or even array of objects). We must design the structure to be easy to use and in the same time to capture all the requered information.',
    id: 38,
  },
  {
    module: 'Core',
  ...
  },
  {
    ...
    answer:
      'Linked list we call a linear collection of data elements whose order is not given by their physical placement in memory. Instead, each element points to the next(sometimes even the previous, first, last etc.). It is a data structure consisting of a collection of nodes which together represent a sequence.',
    id: 40,
  },
  // --------------------------------
  // OOP
...
const dropDownModule = document.querySelector('.selectModule');
...
const removeEventListener = (element, funk) =>
  element.removeEventListener('click', funk);

const getQuestion = (x = 0) =>
  (qField.textContent = desiredQuestions[x].question);

const showAnswer = () => {
  let current = 0;
  for (let i = 0; i < desiredQuestions.length; i++) {
    if (desiredQuestions[i].question === qField.textContent) current = i;
  }
...
const nextQuestion = () => {
  ...
  aField.textContent = '';
  if (current < desiredQuestions.length - 1) return getQuestion(current + 1);
  else {
    qField.textContent =
      'No more Questions from this module, select new module for more questions :)';
